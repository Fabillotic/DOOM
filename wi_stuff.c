...
void WI_initAnimatedBack(void) {
	...
	for(i = 0; i < NUMANIMS[wbs->epsd]; i++) {
		...
		else if(a->type == ANIM_LEVEL) a->nexttic = bcnt + 1;
	}
}
...
void WI_drawAnimatedBack(void) {
	...
	for(i = 0; i < NUMANIMS[wbs->epsd]; i++) {
		...
		if(a->ctr >= 0) V_DrawPatch(a->loc.x, a->loc.y, FB, a->p[a->ctr]);
	}
}
...
int WI_drawNum(int x, int y, int n, int digits) {
	...
	if(neg) V_DrawPatch(x -= 8, y, FB, wiminus);
...
void WI_updateShowNextLoc(void) {
	...
	if(!--cnt || acceleratestage) WI_initNoState();
...
void WI_drawShowNextLoc(void) {
	...
	if((gamemode != commercial) || wbs->next != 30) WI_drawEL();
}
...
void WI_drawNetgameStats(void) {
	...
	for(i = 0; i < MAXPLAYERS; i++) {
		...
		if(i == me) V_DrawPatch(x - SHORT(p[i]->width), y, FB, star);
		...
		if(dofrags) WI_drawNum(x, y + 10, cnt_frags[i], -1);
...
void WI_loadData(void) {
	...
	if(gamemode == commercial) strcpy(name, "INTERPIC");
	...
	if(gamemode == commercial) {
		NUMCMAPS = 32;
		lnames =
		    (patch_t **) Z_Malloc(sizeof(patch_t *) * NUMCMAPS, PU_STATIC, 0);
		...
		lnames =
		    (patch_t **) Z_Malloc(sizeof(patch_t *) * NUMMAPS, PU_STATIC, 0);
		for(i = 0; i < NUMMAPS; i++) {
	...
	if(french) {
		// "items"
		if(netgame && !deathmatch) items = W_CacheLumpName("WIOBJ", PU_STATIC);
		else items = W_CacheLumpName("WIOSTI", PU_STATIC);
	}
	else items = W_CacheLumpName("WIOSTI", PU_STATIC);
