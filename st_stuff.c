...
cheatseq_t cheat_powerup[7] = {{cheat_powerup_seq[0], 0},
    {cheat_powerup_seq[1], 0}, {cheat_powerup_seq[2], 0},
    {cheat_powerup_seq[3], 0}, {cheat_powerup_seq[4], 0},
    {cheat_powerup_seq[5], 0}, {cheat_powerup_seq[6], 0}};
	...
	// if a user keypress...
		...
		if(!netgame) {
			...
			if(cht_CheckCheat(&cheat_god, ev->data1)) {
				...
				else plyr->message = STSTR_DQDOFF;
			}
		...
		if(cht_CheckCheat(&cheat_clev, ev->data1)) {
			...
			if((gamemode == retail) && ((epsd > 4) || (map > 9))) return false;
...
void ST_updateWidgets(void) {
	...
	else w_ready.num = &plyr->ammo[weaponinfo[plyr->readyweapon].ammo];
	//{
	// static int tic=0;
	// static int dir=-1;
	// if (!(tic&15))
	...
	for(i = 0; i < MAXPLAYERS; i++) {
		...
		else st_fragscount -= plyr->frags[i];
	}
...
void ST_unloadGraphics(void) {
	...
	// unload gray #'s
	for(i = 0; i < 6; i++) Z_ChangeTag(arms[i][0], PU_CACHE);
