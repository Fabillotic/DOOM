...
void D_ProcessEvents(void) {
	...
	if((gamemode == commercial) && (W_CheckNumForName("map01") < 0)) return;
...
void D_Display(void) {
	...
	// draw pause pic
	if(paused) {
		if(automapactive) y = 4;
		else y = viewwindowy + 4;
...
void D_DoAdvanceDemo(void) {
	...
	else demosequence = (demosequence + 1) % 6;
...
void D_DoomMain(void) {
	...
	if(p) {
		...
		default:
			p = atoi(myargv[p + 1]);
			if(p < 10) sprintf(file, "~" DEVMAPS "cdata/map0%i.wad", p);
			else sprintf(file, "~" DEVMAPS "cdata/map%i.wad", p);
			break;
		}
		D_AddFile(file);
	}
	...
	if(modifiedgame) {
		...
		char name[23][8] = {"e2m1", "e2m2", "e2m3", "e2m4", "e2m5", "e2m6",
		    "e2m7", "e2m8", "e2m9", "e3m1", "e3m3", "e3m3", "e3m4", "e3m5",
		    "e3m6", "e3m7", "e3m8", "e3m9", "dphoof", "bfgga0", "heada1",
		    "cybra1", "spida1d1"};
		int i;
	...
	if(modifiedgame) {
		/*m*/ printf("========================================================="
		             "==================\n"
		             "ATTENTION:  This version of DOOM has been modified.  If "
		             "you would like to\n"
		             "get a copy of the original game, call 1-800-IDGAMES or "
		             "see the readme file.\n"
		             "        You will not receive technical support for "
		             "modified games.\n"
	...
	if(p && p < myargc - 1) {
		...
		else sprintf(file, SAVEGAMENAME "%c.dsg", myargv[p + 1][0]);
		G_LoadGame(file);
	}

	if(gameaction != ga_loadgame) {
		if(autostart || netgame) G_InitNew(startskill, startepisode, startmap);
		else D_StartTitle(); // start up intro loop
	}
