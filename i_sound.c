...
void* synthesize(mus_event_t *events, fluid_synth_t *synth, fluid_sequencer_t *sequencer, int *r_wsize);
unsigned char get_midi_channel(unsigned char channel);
int parse_wav(void *wav_data, int wav_len, void **r_data, int *r_size, short *r_channels, short *r_bps, int *r_freq);
...
mus_event_t *parse_data(char *data) {
	...
	printf("Length: %d, primary channels: %d, secondary channels: %d, instruments: %d\n", len, p_ch, s_ch, n_instr);
...
void* synthesize(mus_event_t *events, fluid_synth_t *synth, fluid_sequencer_t *sequencer, int *r_wsize) {
	...
	for(event = events; event; event = event->next) {
		...
		if(event->type == MUS_PRESS) {
			...
			fluid_event_noteon(fluid_ev, midi_channel, event->data[0], volume[event->channel]);
			...
			fluid_event_set_source(fluid_ev, -1);
			fluid_event_set_dest(fluid_ev, synth_dest);
			fluid_event_pitch_bend(fluid_ev, midi_channel, ((int) event->data[0]) * 64);
			fluid_sequencer_send_at(sequencer, fluid_ev, ticks, 1);
			delete_fluid_event(fluid_ev);
		}
			...
			if(event->data[0] == 0) {
				...
				fluid_event_set_source(fluid_ev, -1);
				fluid_event_set_dest(fluid_ev, synth_dest);
				fluid_event_program_change(fluid_ev, midi_channel, event->data[1]);
				fluid_sequencer_send_at(sequencer, fluid_ev, ticks, 1);
				delete_fluid_event(fluid_ev);
			}
				...
				fluid_event_set_source(fluid_ev, -1);
				fluid_event_set_dest(fluid_ev, synth_dest);
				fluid_event_control_change(fluid_ev, midi_channel, 10, event->data[1]);
				fluid_sequencer_send_at(sequencer, fluid_ev, ticks, 1);
				delete_fluid_event(fluid_ev);
			}
		}
