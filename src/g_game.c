	...
	for(i = 0; i < MAXPLAYERS; i++) {
		if(playeringame[i]) {
			...
			if(netgame && !netdemo && !(gametic % ticdup)) {
				...
				if(players[i].mo) consistancy[i][buf] = players[i].mo->x;
				else consistancy[i][buf] = rndindex;
			}
		}
	}
...
// DOOM Par Times
int pars[4][10] = {{0}, {0, 30, 75, 120, 90, 165, 180, 180, 30, 165},
...
void G_DoCompleted(void) {
	...
	if(gamemode != commercial) switch(gamemap) {
		case 8: gameaction = ga_victory; return;
		case 9:
	...
	if(gamemode == commercial) {
		...
		if(secretexit) wminfo.next = 8; // go to secret level
		else if(gamemap == 9) {
		...
		else wminfo.next = gamemap; // go to next level
	}
	...
	else wminfo.partime = 35 * pars[gameepisode][gamemap];
	...
	if(statcopy) memcpy(statcopy, &wminfo, sizeof(wminfo));
...
void G_DoSaveGame(void) {
	...
	else sprintf(name, SAVEGAMENAME "%d.dsg", savegameslot);
	...
	*save_p++ = 0x1d; // consistancy marker
	...
	if(length > SAVEGAMESIZE) I_Error("Savegame buffer overrun");
...
void G_RecordDemo(char *name) {
	...
	if(i && i < myargc - 1) maxsize = atoi(myargv[i + 1]) * 1024;
...
boolean G_CheckDemoStatus(void) {
	...
	if(timingdemo) {
		endtime = I_GetTime();
		I_Error(
		    "timed %i gametics in %i realtics", gametic, endtime - starttime);
	}
