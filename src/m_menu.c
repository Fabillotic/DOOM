...
menuitem_t MainMenu[] = {{1, "M_NGAME", M_NewGame, 'n'},
    {1, "M_OPTION", M_Options, 'o'}, {1, "M_LOADG", M_LoadGame, 'l'},
...
menuitem_t EpisodeMenu[] = {{1, "M_EPI1", M_Episode, 'k'},
    {1, "M_EPI2", M_Episode, 't'}, {1, "M_EPI3", M_Episode, 'i'},
...
menuitem_t NewGameMenu[] = {{1, "M_JKILL", M_ChooseSkill, 'i'},
    {1, "M_ROUGH", M_ChooseSkill, 'h'}, {1, "M_HURT", M_ChooseSkill, 'h'},
    {1, "M_ULTRA", M_ChooseSkill, 'u'}, {1, "M_NMARE", M_ChooseSkill, 'n'}};
...
menuitem_t OptionsMenu[] = {{1, "M_ENDGAM", M_EndGame, 'e'},
    {1, "M_MESSG", M_ChangeMessages, 'm'}, {1, "M_DETAIL", M_ChangeDetail, 'g'},
...
menuitem_t SoundMenu[] = {{2, "M_SFXVOL", M_SfxVol, 's'}, {-1, "", 0},
...
void M_ReadSaveStrings(void) {
	...
	for(i = 0; i < load_end; i++) {
		...
		else sprintf(name, SAVEGAMENAME "%d.dsg", i);
...
void M_LoadSelect(int choice) {
	...
	else sprintf(name, SAVEGAMENAME "%d.dsg", choice);
	G_LoadGame(name);
	M_ClearMenus();
}
...
void M_ChangeMessages(int choice) {
	...
	else players[consoleplayer].message = MSGON;
...
// M_QuitDOOM
//
int quitsounds[8] = {sfx_pldeth, sfx_dmpain, sfx_popain, sfx_slop, sfx_telept,
    sfx_posit1, sfx_posit3, sfx_sgtatk};

int quitsounds2[8] = {sfx_vilact, sfx_getpow, sfx_boscub, sfx_slop, sfx_skeswg,
    sfx_kntdth, sfx_bspact, sfx_sgtatk};
...
void M_QuitResponse(int ch) {
	...
	if(!netgame) {
		...
		else S_StartSound(NULL, quitsounds[(gametic >> 2) & 7]);
		I_WaitVBL(105);
	}
	I_Quit();
}
...
void M_QuitDOOM(int choice) {
	...
	if(language != english) sprintf(endstring, "%s\n\n" DOSY, endmsg[0]);
	else
		sprintf(endstring, "%s\n\n" DOSY,
...
void M_DrawThermo(int x, int y, int thermWidth, int thermDot) {
	...
	V_DrawPatchDirect(
	    (x + 8) + thermDot * 8, y, 0, W_CacheLumpName("M_THERMO", PU_CACHE));
}
