...
int UDPsocket(void) {
	...
	if(s < 0) I_Error("can't create socket: %s", strerror(errno));
...
void BindToLocalPort(int s, int port) {
	...
	if(v == -1) I_Error("BindToPort: bind: %s", strerror(errno));
}
...
void PacketGet(void) {
	...
	c = recvfrom(insocket, &sw, sizeof(sw), 0, (struct sockaddr *) &fromaddress,
	    &fromlen);
	if(c == -1) {
		if(errno != EWOULDBLOCK) I_Error("GetPacket: %s", strerror(errno));
...
int GetLocalAddress(void) {
	...
	if(v == -1) I_Error("GetLocalAddress : gethostname: errno %d", errno);
...
void I_InitNetwork(void) {
	...
	else doomcom->ticdup = 1;

	if(M_CheckParm("-extratic")) doomcom->extratics = 1;
	else doomcom->extratics = 0;
