...
int filelength(int handle) {
	...
	if(fstat(handle, &fileinfo) == -1) I_Error("Error fstating");
...
void W_Reload(void) {
	...
	for(i = reloadlump; i < reloadlump + lumpcount; i++, lump_p++, fileinfo++) {
		if(lumpcache[i]) Z_Free(lumpcache[i]);
...
void W_Profile(void) {
	...
	for(i = 0; i < numlumps; i++) {
		...
		if(!ptr) {
			...
			if(block->tag < PU_PURGELEVEL) ch = 'S';
			else ch = 'P';
		}
	...
	for(i = 0; i < numlumps; i++) {
		...
		for(j = 0; j < profilecount; j++) fprintf(f, "    %c", info[i][j]);
