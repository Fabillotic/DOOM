...
void P_RecursiveSound(sector_t *sec, int soundblocks) {
	...
	for(i = 0; i < sec->linecount; i++) {
		...
		else other = sides[check->sidenum[0]].sector;
...
seeyou:
	if(actor->info->seesound) {
		...
		case sfx_posit1:
		case sfx_posit2:
		case sfx_posit3: sound = sfx_posit1 + P_Random() % 3; break;

		case sfx_bgsit1:
		case sfx_bgsit2: sound = sfx_bgsit1 + P_Random() % 2; break;
...
void A_Chase(mobj_t *actor) {
	...
	if(actor->movedir < 8) {
		...
		if(delta > 0) actor->angle -= ANG90 / 2;
		else if(delta < 0) actor->angle += ANG90 / 2;
	}
...
void A_VileTarget(mobj_t *actor) {
	...
	fog = P_SpawnMobj(
	    actor->target->x, actor->target->x, actor->target->z, MT_FIRE);
...
void A_Scream(mobj_t *actor) {
	...
	case sfx_podth1:
	case sfx_podth2:
	case sfx_podth3: sound = sfx_podth1 + P_Random() % 3; break;

	case sfx_bgdth1:
	case sfx_bgdth2: sound = sfx_bgdth1 + P_Random() % 2; break;

	default: sound = actor->info->deathsound; break;
	}
