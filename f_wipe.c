...
void wipe_shittyColMajorXform(short *array, int width, int height) {
	...
	for(y = 0; y < height; y++)
		for(x = 0; x < width; x++) dest[x * height + y] = array[y * width + x];
...
int wipe_ScreenWipe(
	...
	int rc;
	static int (*wipes[])(int, int, int) = {wipe_initColorXForm,
	    wipe_doColorXForm, wipe_exitColorXForm, wipe_initMelt, wipe_doMelt,
	    wipe_exitMelt};
